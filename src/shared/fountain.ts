...
/** The block indices XORed into frame `seq`, as a Set — identical on both ends.
 * Returns the Set directly (not an array): the decoder needs a mutable set for
 * peeling and the encoder just iterates, so building one Set here avoids the old
 * Set→array→Set round-trip (3 allocations per frame → 1). The rnd() draw sequence
 * is unchanged, so sender/receiver streams stay bit-identical. */
function frameIndices(k: number, cdf: Float64Array, sessionId: number, seq: number): Set<number> {
  ...
  if (d > k >> 3) {
    // large degree: partial Fisher–Yates over an identity array (distinct by
    // construction, so the Set just collects the first d shuffled entries).
...
function xorInto(dst: Uint32Array, src: Uint32Array): void {
  // No `>>> 0`: storing into a Uint32Array already applies ToUint32, so the XOR
  // result is coerced unsigned on assignment — the extra shift was dead work.
