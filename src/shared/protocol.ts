...
//  12  u32  totalLen    TRANSMITTED payload length (the container — compressed if any)
//  16  u32  payloadFnv  FNV-1a of the transmitted payload — verified on completion
//  20  u32  origLen     ORIGINAL uncompressed file size, for display (0 if unknown)
//
// NOTE: sessionId@2 and seq@4 are read by the native decoder (camera_android.rs)
// for dedup — never move them. New fields append at the end (offset 20+).
...
export interface FrameHeader {
  ...
  k: number;
  blockLen: number;
  totalLen: number; // transmitted (possibly compressed) payload size
  payloadFnv: number;
  origLen: number; // original uncompressed file size (for the UI)
}
...
export function packFrame(h: FrameHeader, block: Uint8Array): Uint8Array {
  ...
  dv.setUint32(20, h.origLen, true);
